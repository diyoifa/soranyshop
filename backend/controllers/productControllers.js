...
export const updateProductWithStockUpdate = async (req, res, next) => {
  const productId = req.body.id;
  const quantityToUpdate = req.body.quantity;

  if (!quantityToUpdate || isNaN(quantityToUpdate) || quantityToUpdate <= 0) {
  ...
  try {
    ...
    if (product.stock < quantityToUpdate) {
      return next(new ErrorHandler("Insufficient stock.", 400));
    }

    product.stock -= quantityToUpdate;
    await product.save();
