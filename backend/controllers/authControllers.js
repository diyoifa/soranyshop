...
// console.log("🚀 ~ file: authControllers.js:23 ~ updateProfile ~ req body:", req.body)
  const newUserData = {
    name: req.body.name,
  ...
  if (req.files.length > 0) {
    const uploader = async (path) => await uploads(path, "soranyshop/avatars");
...
export const updatePassword = async (req, res, next) => {
  ...
  if (!isPasswordMatched) {
    return next(new ErrorHandler("Contraseña anterior incorrecta", 400));
  }
